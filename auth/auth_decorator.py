...
def check_permissions(permission, payload):
    if 'permissions' not in payload:
        raise AuthError({'code': 'payload_permissions_missing', 'description':
            'Payload permissions are expected'}, 401)
    elif permission not in payload['permissions']:
        raise AuthError({'code': 'not_allowed', 'description':
            'Permissions do not include this action.'}, 403)
    return True
